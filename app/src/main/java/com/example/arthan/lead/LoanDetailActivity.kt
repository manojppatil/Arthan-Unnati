...
import kotlinx.android.synthetic.main.activity_loan_detail.btn_submit
...
//            }
    ...
    private fun checkForProceed(): Boolean {
        var proceed = false
//        if(et_years.length()>0)
//        {

        val maxTenure = intent.getStringExtra(ArgumentKey.maxTenure)
        val minTenure = intent.getStringExtra(ArgumentKey.minTenure)
        val minLoan = intent.getStringExtra("minLoan")
        val maxLoan = intent.getStringExtra("maxLoan")
        if (maxTenure != null && minTenure != null) {


            val maxValue = intent.getStringExtra(ArgumentKey.maxTenure).toInt()
            val minValue = intent.getStringExtra(ArgumentKey.minTenure).toInt()
            val minLoanAmnt = minLoan.toDouble()
            val maxLoanAmnt = maxLoan.toDouble()
            val duration = (et_years.tag as? Int)!! * 12 + (et_months.tag as? Int)!!
            proceed =
                loan_amount_input?.text?.isNotEmpty() == true
                    ...
                    .isNotEmpty() && (netMonthlyIncomet?.text.toString().toLong() > 0)) &&
                        minLoan != null && maxLoan != null && loan_amount_input.text.toString()
                    .toDouble() >= minLoanAmnt
                        && (loan_amount_input.text.toString()
                    .toDouble()) <= maxLoanAmnt && duration <= maxValue && duration >= minValue
            return proceed
        }
        /*if (duration >=maxValue||duration<=minValue) {
//                    Toast.makeText(this,"Please select tenure range between ${minTenure} and ${maxTenure}",Toast.LENGTH_LONG).show()
        } else {
             ...
             if(et_years.length()>0)
                 {
                     ...
                     if(maxTenure!=null&&minTenure!=null) {
                         ...
                         val maxValue =intent.getStringExtra(ArgumentKey.maxTenure) as Int
                         val minValue =intent.getStringExtra(ArgumentKey.minTenure) as Int
                         val duration=(et_years.tag as? Int)!!*12 + (et_months.tag as? Int)!!
                         if (duration >=maxValue||duration<=minValue) {
                             Toast.makeText(this,"Please select tenure range between ${minTenure} and ${maxTenure}",Toast.LENGTH_LONG).show()
...
//            if(proceed) {
            ...
            if (maxTenure != null && minTenure != null) {
                ...
                val maxLoanAmnt = maxLoan.toDouble()
                val duration = (et_years.tag as? Int)!! * 12 + (et_months.tag as? Int)!!
            if (loan_amount_input?.text?.isNotEmpty()!!) {
                proceed = true
            }else{
                proceed=false
                Toast.makeText(this,"Please enter a valid loan amount ",Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
            if (business_turnover_input?.text.toString()
                    .isNotEmpty() && (business_turnover_input?.text.toString().toLong() > 0)
            ) {
                proceed = true
            }else
            {
                proceed=false
                Toast.makeText(this,"Please enter a valid business turnover amount ",Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
            if (net_profit_margin_input?.text.toString()
                    .isNotEmpty() && (net_profit_margin_input?.text.toString().toLong() > 0)
            ) {
                         proceed=true
            }else
            {
                proceed=false
                Toast.makeText(this,"Please enter a valid net profit value ",Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
            if (existing_loan_input?.text?.isNotEmpty() == true && existing_loan_obligation_input?.text?.isNotEmpty() == true)
            {
                proceed=true
            }else
            {
                proceed=false
                Toast.makeText(this,"Please enter a valid existing loan  ",Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
                if(netMonthlyIncomet?.text.toString()
                    .isNotEmpty() && (netMonthlyIncomet?.text.toString().toLong() > 0)
            ) {
                proceed=true
            ...
            if(minLoan != null && maxLoan != null && loan_amount_input.text.toString()
                    .toDouble() >= minLoanAmnt && loan_amount_input.text.toString().toDouble()
             <= maxLoanAmnt)
            {
              proceed=true
            }
                ...
                proceed=false
                Toast.makeText(this,"Please enter a  loan amount between $minLoan and $maxLoan ",Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
            if (duration in minValue..maxValue) {
            ...
            {
                proceed=false
                Toast.makeText(this,"Please enter a valid tenure  ",Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
...
//        et_years.setText(loanDetails.yea)
                    ...
                    getStaticSpinnerData(
                        security_jurisdiction_spinner,
                        loanDetails?.securityJurisdiction
                    )
...
companion object {
    fun startMe(
        ...
        context?.startActivity(Intent(context, LoanDetailActivity::class.java).apply {
            ...
            putExtra(ArgumentKey.maxTenure, maxTenure)
            putExtra(ArgumentKey.minTenure, minTenure)
            putExtra("minLoan", minLoan)
            putExtra("maxLoan", maxLoan)
        })
}
